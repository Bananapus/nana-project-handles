...
contract JBProjectHandles is IJBProjectHandles, ERC2771Context {
    ...
    /// @param chainIds The chain IDs of the networks the project is on.
    /// @return handles The project's verified handles, corresponding to the `chainIds` array.
    function handlesOf(
        uint256 projectId,
        address setter,
        uint256[] calldata chainIds
    ) external view override returns (string[] memory handles) {
        // Initialize the handles array.
        handles = new string[](chainIds.length);
        ...
        for (uint256 i; i < chainIds.length; i++) {
            ...
            if (ensNameParts.length == 0) continue;
            ...
            if (textResolver == address(0)) continue;
            ...
            handles[i] = _formatHandle(ensNameParts);
        }
    }
